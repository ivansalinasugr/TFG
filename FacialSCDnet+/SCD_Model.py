...
class SCDModel():
	...
	def create_model(self):
		if self.config['backbone'] == 'vgg':
			...
			backbone = models.inception_v3(weights=models.Inception_V3_Weights.DEFAULT)
			num_features = backbone.fc.in_features
			# Freeze all layers
			for param in backbone.parameters():
				param.requires_grad = False
	...
	def load_weights_only(self):
		...
		#self.model.load_state_dict(torch.load('SCDnet/synth_trained_models/BEST_f35_390f5.hdf5'))
