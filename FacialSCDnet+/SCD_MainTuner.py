...
def load_config(file_path, args):
	...
	config['model_file']=os.path.abspath(args.src_root + "/best_models/best_model_"+ config['backbone'] + ".pth")
...
def objective(trial, args, hparams):
	...
	hparams['initial_learning_rate'] = trial.suggest_float('initial_learning_rate', 1e-6, 9e-3, log=True)
	hparams['optimizer'] = trial.suggest_categorical('optimizer', ['adam', 'sgd'])
	hparams['patience'] = trial.suggest_categorical('patience', [2, 3, 4])
	...
	hparams['dropout'] = trial.suggest_categorical('dropout', [0.0, 0.1, 0.2])
...
if __name__ == "__main__":
	...
	study.optimize(lambda trial: objective(trial, args, hparams), n_trials=20) 
